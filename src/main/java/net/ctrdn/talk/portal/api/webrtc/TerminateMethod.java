...
public class TerminateMethod extends DefaultApiMethod {

    public TerminateMethod(ProxyController proxyController) {
        super(proxyController, "webrtc.terminate");
    }
    ...
    public JsonObjectBuilder execute(ProxyController proxyController, HttpServletRequest request, HttpServletResponse response) throws ApiMethodException {
        ...
        if (session.getCallerDao().getObjectId().equals(userDao.getObjectId())) {
            ...
            throw new ApiMethodException("Invalid session state for termination");
        }
